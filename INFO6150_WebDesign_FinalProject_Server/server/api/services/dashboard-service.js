  ...
  try {
    ...
    if (timeline === "daily") {
      for (let i = 0; i < activities.length; i++) {
        ...
        let stat = {};
        stat.activityId = activity.activityMasterId
          ? activity.activityMasterId
          : "1";
        stat.userActivityId = activity.userActivityId
        ...
        stat.stat1Unit = activity.preferredValueUnit
          ? activity.preferredValueUnit
          : "";
        stat.stat2Unit = activity.preferredTimeUnit
          ? activity.preferredTimeUnit
          : "";
      ...
      for (let i = 0; i < activities.length; i++) {
        ...
        if (activity.history.length > 0) {
          const days = DateUtils.getDays(
            activity.activityStartDate,
            activity.history[activity.history.length - 1].dateTime
          );
          ...
          while (weeks > 0) {
            ...
            for (let k = historyCount; k >= 0; k--) {
              ...
              if (
                (history.dateTime <= endDate && history.dateTime > startDate) ||
                (weeks === 1 && history.dateTime >= startDate)
              ) {
          ...
          stat.stat1 = Math.round(totalValueSum / Math.ceil(days / 7)).toFixed(
            1
          );
          stat.stat2 = Math.round(
            totalDurationSum / Math.ceil(days / 7)
          ).toFixed(1);
        }
        exercise.push(stat);
      }
        ...
        if (activity.history.length > 0) {
          ...
          while (months > 0) {
            ...
            for (let k = historyCount; k >= 0; k--) {
              ...
              if (
                (history.dateTime <= endDate && history.dateTime > startDate) ||
                (months === 1 && history.dateTime >= startDate)
              ) {
          ...
          stat.stat1 = Math.round(totalValueSum / Math.ceil(days / 30)).toFixed(
            1
          );
          stat.stat2 = Math.round(
            totalDurationSum / Math.ceil(days / 30)
          ).toFixed(1);
        }
        exercise.push(stat);
      }
        ...
        if (activity.history.length > 0) {
          ...
          while (years > 0) {
            ...
            for (let k = historyCount; k >= 0; k--) {
              ...
              if (
                (history.dateTime <= endDate && history.dateTime > startDate) ||
                (years === 1 && history.dateTime >= startDate)
              ) {
          ...
          stat.stat1 = Math.round(
            totalValueSum / Math.ceil(days / 365)
          ).toFixed(1);
          stat.stat2 = Math.round(
            totalDurationSum / Math.ceil(days / 365)
          ).toFixed(1);
        }
        exercise.push(stat);
      }
...
export const getHealthStats = async (uuid, timeline) => {
  try {
    ...
    const activities = await userHealthService.getAllHealthActivitiesForUser(
      uuid
    );
  ...
  try {
    ...
    if (timeline === "daily") {
      for (let i = 0; i < activities.length; i++) {
        ...
        stat.value = 0;
        stat.unit = activity.preferredValueUnit
          ? activity.preferredValueUnit
          : "";
        if (activity.history.length > 0) {
          let history = activity.history;
          ...
          dailyTimeStats = dailyTimeStats.sort(function (a, b) {
            return a - b;
          });
          dailyValueStats = dailyValueStats.sort(function (a, b) {
            return a - b;
          });
          stat.value = Math.round(
            dailyValueStats[dailyValueStats.length - 1]
          ).toFixed(1);
        }
        exercise.push(stat);
      }
      ...
      for (let i = 0; i < activities.length; i++) {
        ...
        if (activity.history.length > 0) {
          ...
          weeklyTimeAverageArray = weeklyTimeAverageArray.sort(function (a, b) {
            return a - b;
          });
          weeklyValueAverageArray = weeklyValueAverageArray.sort(function (
            a,
            b
          ) {
            return a - b;
          ...
          stat.value = Math.round(
            weeklyValueAverageArray[weeklyValueAverageArray.length - 1]
          ).toFixed(1);
          stat.action = activity.actionText;
        }
        exercise.push(stat);
      ...
      for (let i = 0; i < activities.length; i++) {
        ...
        if (activity.history.length > 0) {
          ...
          monthlyTimeAverageArray = monthlyTimeAverageArray.sort(function (
            a,
            b
          ) {
            return a - b;
          });
          monthlyValueAverageArray = monthlyValueAverageArray.sort(function (
            a,
            b
          ) {
            return a - b;
          ...
          stat.value = Math.round(
            monthlyValueAverageArray[monthlyValueAverageArray.length - 1]
          ).toFixed(1);
          stat.unit = activity.preferredValueUnit
            ? activity.preferredValueUnit
            : "";
      ...
      for (let i = 0; i < activities.length; i++) {
        ...
        if (activity.history.length > 0) {
          ...
          yearlyTimeAverageArray = yearlyTimeAverageArray.sort(function (a, b) {
            return a - b;
          });
          yearlyValueAverageArray = yearlyValueAverageArray.sort(function (
            a,
            b
          ) {
            return a - b;
          ...
          stat.value = Math.round(
            yearlyValueAverageArray[yearlyValueAverageArray.length - 1]
          ).toFixed(1);
          stat.unit = activity.preferredValueUnit
            ? activity.preferredValueUnit
            : "";
